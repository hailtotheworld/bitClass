...
-- 연봉에 +comm, 주의! comm의 값 중에는 null이 있다.
...
-- null 값을 치환해주는 nvl함수
-- nvl(컬럼명, 치환하는 데이터) => 컬럼의 값중 null 값은 치환 값으로 출력
...
-- 별칭을 이용해서 컬럼이름 사용
select ename as "사원이름", sal, sal*12+comm as "연봉"
from emp;

select ename name, sal "sal", sal*12+comm salofYear
from emp;
...
from emp
where ename='FORD'; --★문자열 비교시 반드시 작은 따옴표 사용!!! 대소문자 구별!!!!!★
