...
@Data
@Entity //테이블과 매핑되서 관리되는 객체라는 의미이다. JPA가 사용하는 객체라는 뜻이다. 이 에노테이션이 있어야 JPA가 인식할 수 있다.
// 이렇게 @Entity 가 붙은 객체를 JPA에서는 엔티티라 한다.
//@Table(name = "item") //객체명과 테이블명이 같으면 생략해도 된다
public class Item {

    @Id //테이블의 PK와 해당 필드를 매핑한다.
    @GeneratedValue(strategy = GenerationType.IDENTITY) // IDENTITY전략은 autoincrement처럼 DB에서 값을 넣어주는 전략이다
    private Long id;
...
-스프링 부트와 통합해서 사용하면 필드 이름을 테이블 컬럼 명으로 변경할 때 객체 필드의 카멜 케이스를 테이블 컬럼의 언더스코어로 자동으로 변환해준다.
  itemName → item_name , 따라서 위 예제의 @Column(name = "item_name") 를 생략해도된다.*/
    ...
    private Integer price; //컬럼명과 같으면 @Column생략가능하다
