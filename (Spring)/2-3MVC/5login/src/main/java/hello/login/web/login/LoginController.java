...
//    @PostMapping("/login")
    public String loginV3(@Validated @ModelAttribute("loginForm") LoginForm form, BindingResult bindingResult, HttpServletResponse response, HttpServletRequest request) {
...
//        sessionManager.createSession(loginMember, response);
    ...
    @PostMapping("/login")
    public String loginV4(@Validated @ModelAttribute("loginForm") LoginForm form, BindingResult bindingResult, HttpServletRequest request,
...
//    @PostMapping("/logout")
    ...
    public String logoutV3(HttpServletResponse response, HttpServletRequest request) {
        ...
        if(session != null) {
            session.invalidate();  // invalidate (서류·계약·선거 등을) 무효화하다
        }
        return "redirect:/";
    }
